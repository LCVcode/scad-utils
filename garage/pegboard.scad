...
module measuring_tape_holder() {
  ...
  tolerance = 1.2;
  slot_width = 38;
  holder_height = inches(1);


  // Profile of the tape measure itself.  Includes extra bits to build the slot.
  module tool_profile() {
    x1 = tool_width / 2 + tolerance;
    x2 = slot_width / 2 + tolerance;
    x3 = x2 + wall_thickness;
    y1 = tool_thickness / 2 + tolerance;
    y2 = y1 + wall_thickness;
    y3 = y2 + 1;
    r1 = tolerance * 3;
    r2 = wall_thickness / 2 - tolerance;
  ...
  module holder_profile() {
    x = tool_width / 2 + wall_thickness;
    y = tool_thickness / 2 + wall_thickness;

    round_poly([
      [0,  y, 0],
      [x,  y, tolerance * 3 + wall_thickness],
      [x, -y, tolerance * 3 + wall_thickness],
      [0, -y, 0],
    ]);
  }
  ...
  union() {
    hanger_backing(width=3);
    translate([0, -tool_thickness/2 - 1.1*tolerance, -holder_height])
      difference() {
        union() {
          rotate([-90, 0, 0])
        ...
        union() {
          rotate([-90, 0, 0])
            ...
            tool_profile();
          copy_mirror_x()
            linear_extrude(holder_height + 1)
            tool_profile();
        }
      }
  }
